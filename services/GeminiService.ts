...
const getResponseTextOrThrow = (text: string | undefined): string => {
    if (!text) {
...
[
  {
    ...
    return JSON.parse(getResponseTextOrThrow(response.text)) as Slider[];
};
...
`;
    ...
    const enrichedData = JSON.parse(getResponseTextOrThrow(response.text)) as { variableName: string; newName: string; newDescription: string; }[];
...
`;
    ...
    return JSON.parse(getResponseTextOrThrow(response.text)) as SliderSuggestion[];
};
...
`;
    ...
    return getResponseTextOrThrow(response.text);
};
...
CRITICAL RULE: For any request that introduces a new visual element or behavior (like ripples, twists, color cycling), you MUST choose "smart_slider". Do not choose "modify_code" for these creative tasks.`;
    ...
    return JSON.parse(getResponseTextOrThrow(response.text)) as ModificationDecision;
};
...
[
  { "variableName": "slider_speed", "newValue": 5.0 }
    ...
    const adjustments = JSON.parse(getResponseTextOrThrow(response.text)) as { variableName: string; newValue: number }[];
...
`;
    ...
    return JSON.parse(getResponseTextOrThrow(response.text));
};
...
`;
    ...
    const rawMods = JSON.parse(getResponseTextOrThrow(response.text)) as Omit<Modulation, 'id'>[];
